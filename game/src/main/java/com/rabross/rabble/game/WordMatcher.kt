...
class WordMatcherImpl : WordMatcher {
    ...
    override suspend fun match(solution: String, attempt: String): List<Int> {
        return if (solution.length == attempt.length) {
            correctPass(solution, attempt).zip(presentPass(solution, attempt)) { v1, v2 ->
                max(v1, v2)
            }
        } else throw IllegalArgumentException("Attempt must match solution size")
    }

    private fun correctPass(word: String, attempt: String) = attempt.mapIndexed { i, char ->
        if (word[i] == char) TOKEN_CORRECT else TOKEN_ABSENT
    }

    private fun presentPass(word: String, attempt: String) = attempt.map { char ->
        if (word.contains(char)) TOKEN_PRESENT else TOKEN_ABSENT
    }
}